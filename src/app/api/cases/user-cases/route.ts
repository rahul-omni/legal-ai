...
export const POST = auth(async (request: NextAuthRequest) => {
  try {
      ...
      try {
        ...
        const existingCase = await prisma.userCase.findFirst({
          where: {
            ...
            bench: normalizeValue(caseData.bench),
            city: normalizeValue(caseData.city),
            caseType: normalizeValue(caseData.caseType),
            OR: [
              { diaryNumber: caseData.diaryNumber },
              ...(caseData.caseNumber ? [{ case_number: normalizeValue(caseData.caseNumber) }] : [])
            ]
          }
        });
        ...
        if (existingCase) {
          ...
          if (existingCase.diaryNumber === caseData.diaryNumber) {
            duplicateReason = "same diary number";
        ...
        const createdCase = await prisma.userCase.create({
          data: {
            ...
            court: caseData.court || "",
            city: normalizeValue(caseData.city),
            district: normalizeValue(caseData.district),
            bench: normalizeValue(caseData.bench),
            courtComplex: normalizeValue(caseData.courtComplex),
            case_number: normalizeValue(caseData.caseNumber),
            courtType: normalizeValue(caseData.courtType),
          }
        });
    ...
    // If all cases were created (no errors)
