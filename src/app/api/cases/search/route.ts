...
export async function GET(request: NextAuthRequest) {
  try {
    ...
    let caseData: any[] = [];
      if (validated.court === "Supreme Court") {
      // Supreme Court: Only search by diaryNumber  court (no scraping)
      const whereConditions = {
        diaryNumber: { equals: fullDiaryNumber, mode: 'insensitive' as const },
        court: { equals: validated.court, mode: 'insensitive'  as const}
      };
      
      console.log("Supreme Court search - Where conditions:", whereConditions);
      caseData = await prisma.caseManagement.findMany({ where: whereConditions });
      console.log("Supreme Court DB result:", caseData.length);
       // Return Supreme Court results immediately
      const response = {
        success: caseData.length > 0,
        message: caseData.length > 0 ? 'Cases found successfully' : 'No cases found',
        data: caseData
      };      
      await logSearchAttempt(validated, { caseData, scrapeResult: null });
     return NextResponse.json(response, { status: caseData.length > 0 ? 200 : 404 });
      
      
    } else if (validated.court === "High Court") {
