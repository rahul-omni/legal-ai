...
const benchOptions = useMemo(() => {
  ...
  // Add NCLT bench options
const ncltBenchOptions = useMemo(() => {
...
const caseTypeOptions = useMemo(() => {
  ...
  // NCLT Court case types
  if (searchParams.court === "Nclt Court" && bench && NCLT_CASE_TYPE_MAPPINGS[bench]) {
    return Object.keys(NCLT_CASE_TYPE_MAPPINGS[bench]).map(caseType => ({
      label: caseType,
      code: NCLT_CASE_TYPE_MAPPINGS[bench][caseType]
    }));
  }
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="space-y-6">
        ...
        {searchParams.court === "High Court" && (
         ...
         {searchParams.court === "Nclt Court" && (
        <div>
          <label className="block text-sm font-medium text-text-light mb-2">
            NCLT Bench <span className="text-error">*</span>
          </label>
          <select
          ...
          >
            <option value="">Select NCLT Bench</option>
            {ncltBenchOptions.map(bench => (
...
{/* Case Type for NCLT Court (with bench-specific options) */}
{searchParams.court === "Nclt Court" && (
  <div>
    <label className="block text-sm font-medium text-text-light mb-2">
      Case Type <span className="text-error">*</span>
    ...
    <select
      ...
      disabled={!searchParams.bench}
      className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary bg-background-light ${
        errors?.caseType ? 'border-error' : 'border-border'
      } ${!searchParams.bench ? 'opacity-50 cursor-not-allowed' : ''}`}
    >
      <option value="">Select Case Type</option>
      {searchParams.bench && caseTypeOptions.length > 0 ? (
        caseTypeOptions.map(opt => (
          <option key={opt.label} value={opt.label}>{opt.label}</option>
        ))
      ) : (
        <option value="">
          {!searchParams.bench 
            ? "Please select an NCLT bench first"
            : "No case types available for selected bench"
          }
        </option>
      )}
    </select>
