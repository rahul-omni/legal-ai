...
import { DollarSign, FileText, Calendar, Download, Printer, Send, Plus, X, Loader2, Check, MessageSquare } from 'lucide-react';
...
export function ClientBilling({ clientId, clientName, clientCompany, clientPhone }: ClientBillingProps) {
  ...
  const [selectedBillForMessage, setSelectedBillForMessage] = useState<Bill | null>(null);
  ...
  const handleItemChange = (index: number, field: string, value: string | number) => {
    ...
    updatedItems[index] = {
      ...updatedItems[index],
      [field]: typeof value === 'string' && field === 'amount' ? parseFloat(value) : value
    };
  ...
  const handleCreateBill = () => {
    if (!newBill.items || newBill.items.some(item => !item.description)) {
      showToast('Please fill in all item descriptions', 'error');
      return;
    }
  ...
  const sendWhatsAppMessage = async () => {
    if (!clientPhone) {
      showToast('Client phone number is not available', 'error');
      return;
    }
    ...
    try {
      ...
      if (selectedBillForMessage && selectedBillForMessage.status === 'Draft') {
        const updatedBills = bills.map(bill => 
          bill.id === selectedBillForMessage.id ? { ...bill, status: 'Sent' as const } : bill
        );
        setBills(updatedBills);
      }
      ...
      showToast(error instanceof Error ? error.message : 'Failed to send WhatsApp message', 'error');
    } finally {
      ...
      {bills.length === 0 ? (
        ...
        <div className="border rounded-md overflow-hidden">
          ...
          <div className="divide-y divide-gray-100">
            {bills.map(bill => (
              <div key={bill.id} className="grid grid-cols-12 gap-2 py-1.5 px-3 text-xs items-center">
                <div className="col-span-3 font-medium">{bill.billNumber}</div>
                <div className="col-span-2">{new Date(bill.billDate).toLocaleDateString()}</div>
                <div className="col-span-2">{new Date(bill.dueDate).toLocaleDateString()}</div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-lg">
            <div className="flex justify-between items-center p-3 border-b">
              <h3 className="text-base font-semibold">New Bill for {clientName}</h3>
              <button 
                ...
                <div className="border rounded-md overflow-hidden">
                  ...
                  <div className="divide-y divide-gray-100">
                    {newBill.items?.map((item, index) => (
                      <div key={index} className="grid grid-cols-12 gap-2 py-1.5 px-2 text-xs items-center">
                    ...
                    <div className="flex justify-end">
                      <div className="w-48">
                        <div className="flex justify-between py-1 text-xs font-medium">
                          <span>Total:</span>
                          <span>₹{calculateTotal(newBill.items).toLocaleString()}</span>
                        </div>
                      </div>
                    </div>
                  </div>
            ...
            <div className="p-4">
              <div className="mb-4">
                ...
                <div className="font-medium text-sm flex items-center gap-2">
                  <MessageSquare className="w-4 h-4 text-green-500" />
                  {clientName} {clientPhone ? `(${clientPhone})` : '(No phone number)'}
                </div>
                {!clientPhone && (
              ...
              <div className="mb-4">
                ...
                <div className="bg-gray-50 p-2 rounded border text-sm">
                  <div className="flex justify-between mb-1">
                    ...
                    <span className="font-medium">{selectedBillForMessage.billNumber}</span>
                  </div>
                  <div className="flex justify-between mb-1">
                    <span className="text-gray-600">Amount:</span>
                    <span className="font-medium">₹{selectedBillForMessage.total.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Due Date:</span>
                    <span className="font-medium">{new Date(selectedBillForMessage.dueDate).toLocaleDateString()}</span>
                  </div>
                </div>
              </div>
